...
**Read "Existing Reference" from the document first** (step 5 wrote it), and do
not ask again what it already answers. Where it names a running product, a
design file or a brand:

- **Go and read the real values** before proposing anything. Look for
  `tokens.css`, `theme.css`, `variables.css`, `globals.css`, a
  `tailwind.config.*`, or a `@theme` block, exactly as the design-mock workflow
  does. Record the tokens the product ALREADY uses.
- Inventing a palette beside an existing product produces a foundation that
  argues for colours nobody chose and that no screen can be built from.
- **If the project defines no tokens anywhere, say so plainly** rather than
  quietly inventing some: "this project defines no design tokens, so what
  follows is a proposal, not a reading."

Only where the reference says nothing exists do you ask:
...
### 2. Draw the Style Plate

A palette described in words cannot be judged. Draw ONE page showing the
foundation applied: the colours as swatches with their token names, the type
scale in real sentences, the spacing scale, and one small piece of real
interface using them together.

**Draw it by following:**
{project-root}/.nexus/method/workflows/design-mock/workflow.md. Save it under
`.nexus/assets/`, and LIST it in the `artifacts:` of this workflow's output
document, or the person never sees it.

Say: "I've drawn the visual foundation at
`.nexus/assets/ux-visual-foundation.html` and listed it on this document, so
you can open it here and judge the colours and type as they will actually look,
...
- Read fully and follow: {project-root}/.nexus/method/core/workflows/advanced-elicitation/workflow.md with the current visual foundation content
...
- Read fully and follow: {project-root}/.nexus/method/core/workflows/party-mode/workflow.md with the current visual foundation
...
- Load `{project-root}/.nexus/method/workflows/2-plan-workflows/create-ux-design/steps/step-09-design-directions.md`
...
Write content to file immediately after generation. Load `{project-root}/.nexus/method/workflows/2-plan-workflows/create-ux-design/steps/step-09-design-directions.md` to generate design direction mockups.
