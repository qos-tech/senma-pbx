...
1. Use the glob pattern to find ALL `.md` files (e.g., `{docs_dir}/*architecture*/*.md`).
...
Use best judgment: load the documents whose index entry says they bear on THIS work. Loading a document on a 5% chance of relevance is how a run spends 35.4% of its context on bytes it never uses, so judge from the index entry rather than loading to find out.
...
**When in doubt, load the SECTION, not the document** -- context is finite, and a section you loaded and did not use crowds out the one you needed. See `{project-root}/.nexus/method/references/context-budget.md`.
...
1. Attempt a glob match on the "whole" pattern (e.g., `{docs_dir}/*prd*.md`).
2. If matches are found, load them **within the context budget**: read fully and follow
   `{project-root}/.nexus/method/references/context-budget.md`. An unsharded document is the
   expensive case, so take the frontmatter and the headings first, then the one section you
   will actually use. Load a whole body only when you have a reason you could say out loud.
