...
deferred_work_file: '{project-root}/.nexus/specs/{active_issue}/deferred-work.md'
...
- Active specs (`ready-for-dev`, `in-progress`, `in-review`) under `{project-root}/.nexus/specs/{active_issue}/`? → List them and HALT. Ask user which to resume (or `[N]` for new).
...
- Unformatted spec or intent file lacking `status` frontmatter under `{project-root}/.nexus/specs/{active_issue}/`? → Suggest to the user to treat its contents as the starting intent for this workflow. DO NOT attempt to infer a state and resume it.

## INSTRUCTIONS

1. Load context.
   - List files in `{project-root}/.nexus/docs/` and `{project-root}/.nexus/specs/{active_issue}/`. Read `{project-root}/.nexus/board.yaml` to see where this issue sits (read-only: the app is the board's only writer).
...
5. Generate `spec_file` path:
   - Derive a valid kebab-case spec-key from the clarified intent.
   - If `{project-root}/.nexus/specs/{active_issue}/{spec-key}.md` already exists, append `-2`, `-3`, etc.
   - Set `spec_file` = `{project-root}/.nexus/specs/{active_issue}/{spec-key}.md`.
6. Route:
