...
# TASK-0035E2: materialize Apache listen port + WS backend from env every
# start. Bridge/dev defaults preserve historical Listen 80 and
# ws://asterisk:8088/ws. Host-network pilot uses APACHE_HTTP_PORT=8080
# and ws://127.0.0.1:8088/ws (see compose.host.yaml).
APACHE_HTTP_PORT="${APACHE_HTTP_PORT:-80}"
ASTERISK_WS_BACKEND="${ASTERISK_WS_BACKEND:-ws://asterisk:8088/ws}"
APACHE_SITE=/etc/apache2/sites-available/000-default.conf
APACHE_TEMPLATE=/etc/senma/apache-mag.conf.template
if [ -f "$APACHE_TEMPLATE" ]; then
    sed \
        -e "s|__APACHE_HTTP_PORT__|${APACHE_HTTP_PORT}|g" \
        -e "s|__ASTERISK_WS_BACKEND__|${ASTERISK_WS_BACKEND}|g" \
        "$APACHE_TEMPLATE" > "$APACHE_SITE"
fi
# ports.conf: HTTP on the configured port; keep 443 for optional
# SENMA-owned TLS termination (external mode may leave it unused).
cat > /etc/apache2/ports.conf <<EOF
# Generated by docker/entrypoint.sh (TASK-0035E2) — do not edit by hand.
