...
- `active_issue` = the epic's issue key (`<ISSUE-KEY>`, e.g. `epic-2`) this retrospective is scoped to
- `date` as system-generated current datetime
- YOU MUST ALWAYS SPEAK OUTPUT in your Agent communication style with the config `{communication_language}`

### Paths

- `installed_path` = `{project-root}/.nexus/method/workflows/4-implementation/retrospective`
- `board` = `.nexus/board.yaml` (READ ONLY: the app is the board's only writer)
- `issue_file` = `.nexus/issues/<ISSUE-KEY>.md` (the epic's issue card)

### Input Files

| Input | Description | Path Pattern(s) | Load Strategy |
|-------|-------------|------------------|---------------|
| epics | The completed epic for retrospective | whole: `{docs_dir}/*epic*.md`, sharded_index: `{docs_dir}/*epic*/index.md`, sharded_single: `{docs_dir}/*epic*/epic-{{epic_num}}.md` | SELECTIVE_LOAD |
| previous_retrospective | Previous epic's retrospective (optional) | `.nexus/specs/epic-{{prev_epic_num}}/epic-{{prev_epic_num}}-retro*.md` | SELECTIVE_LOAD |
| architecture | System architecture for context | whole: `{docs_dir}/*architecture*.md`, sharded: `{docs_dir}/*architecture*/*.md` | FULL_LOAD |
| prd | Product requirements for context | whole: `{docs_dir}/*prd*.md`, sharded: `{docs_dir}/*prd*/*.md` | FULL_LOAD |
| document_project | Brownfield project documentation (optional) | sharded: `{docs_dir}/*.md` | INDEX_GUIDED |

### Required Inputs

- `agent_manifest` = `{project-root}/.nexus/method/agents/` (the nine agent definitions)
...
<output>
Bob (Scrum Master): "Welcome to the retrospective, {user_name}. Let me help you identify which epic we just completed. I'll check the board first, but you're the ultimate authority on what we're reviewing today."
</output>

<action>PRIORITY 1: Check {board} first</action>

<action>Load the FULL file: {board}</action>
<action>Read ALL items</action>
<action>Find the highest epic number whose epic issue sits in the done column, or whose specs are all done</action>
<action>Extract epic number from epic issue keys like "epic-X" or spec keys like "X-Y-story-name"</action>
<action>Set {{detected_epic}} = highest epic number found with completed specs</action>

<check if="{{detected_epic}} found">
  <action>Present finding to user with context</action>

  <output>
Bob (Scrum Master): "Based on the board, it looks like Epic {{detected_epic}} was recently completed. Is that the epic you want to review today, {user_name}?"
  </output>
...
<check if="{{detected_epic}} NOT found on the board">
  <action>PRIORITY 2: Ask user directly</action>

  <output>
Bob (Scrum Master): "I'm having trouble detecting the completed epic from the board. {user_name}, which epic number did you just complete?"
  </output>
...
<check if="{{epic_number}} still not determined">
  <action>PRIORITY 3: Fallback to the specs folder</action>

<action>Scan `.nexus/specs/` for the highest numbered spec files</action>
<action>Extract epic numbers from spec filenames (pattern: X-Y-story-name.md, grouped under their issue folder)</action>
<action>Set {{detected_epic}} = highest epic number found</action>

  <output>
Bob (Scrum Master): "I found specs for Epic {{detected_epic}} in the specs folder. Is that the epic we're reviewing, {user_name}?"
  </output>
...
<action>Find all specs for epic {{epic_number}} on the board and in `.nexus/specs/`:

- Look for spec keys starting with "{{epic_number}}-" (e.g., "1-1-", "1-2-", etc.) under the epic's issue(s)
- The epic issue itself is "epic-{{epic_number}}"; its retro spec (if any) is "epic-{{epic_number}}-retro"
  </action>

<action>Count total specs found for this epic</action>
<action>Count specs with `done: true`</action>
<action>Collect list of pending spec keys (`done: false`)</action>
<action>Determine if complete: true if all specs are done, false otherwise</action>
...
<action>For each spec in epic {{epic_number}}, read the complete spec file from `.nexus/specs/<ISSUE-KEY>/{{epic_number}}-{{story_num}}-*.md` (the issue(s) grouping this epic's specs)</action>
...
<check if="{{prev_epic_num}} >= 1">
  <action>Search for previous retrospectives using pattern: `.nexus/specs/epic-{{prev_epic_num}}/epic-{{prev_epic_num}}-retro*.md`</action>
...
<action>Check if file exists: {docs_dir}/epic*/epic-{{next_epic_num}}.md</action>

<check if="sharded epic file found">
  <action>Load {docs_dir}/*epic*/epic-{{next_epic_num}}.md</action>
  <action>Set {{next_epic_source}} = "sharded"</action>
</check>

**Fallback to whole document:**
<check if="sharded epic not found">
<action>Check if file exists: {docs_dir}/epic*.md</action>
...
Bob (Scrum Master): "Epic {{epic_number}} sits in the done column on the board, but is it REALLY done?"
...
<step n="11" goal="Save Retrospective and Signal the Board">

<action>Ensure the epic's specs folder exists: `.nexus/specs/epic-{{epic_number}}/`</action>
...
<action>Set filename: `.nexus/specs/epic-{{epic_number}}/epic-{{epic_number}}-retro.md`</action>
<action>Save retrospective document</action>

<output>
✅ Retrospective document saved: .nexus/specs/epic-{{epic_number}}/epic-{{epic_number}}-retro.md
</output>

<critical>The board has one writer: the app. Do NOT edit board.yaml. Record the retrospective by its saved document and an issue comment; the app reacts.</critical>
<action>Append a comment to the epic's issue frontmatter `.nexus/issues/epic-{{epic_number}}.md`: `who: "agent:sm"`, `at: {date}`, `text: "Retrospective complete for epic {{epic_number}}; summary at epic-{{epic_number}}-retro.md."`</action>

<check if="the epic issue file exists">
  <output>
✅ Retrospective recorded: comment appended to .nexus/issues/epic-{{epic_number}}.md

Retro document: epic-{{epic_number}}-retro.md
The app marks the retrospective done in reaction to the saved document and comment.
</output>
</check>

<check if="the epic issue file not found">
  <output>
⚠️ Could not append the retrospective comment: .nexus/issues/epic-{{epic_number}}.md not found

Retrospective document was saved successfully, but the epic's issue file may need to be created so the app can mark the retro done.
</output>
</check>
...
- Retrospective saved: .nexus/specs/epic-{{epic_number}}/epic-{{epic_number}}-retro.md
...
1. **Review retrospective summary**: .nexus/specs/epic-{{epic_number}}/epic-{{epic_number}}-retro.md
