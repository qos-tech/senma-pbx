...
**Plan-code-review:** Launch three subagents without conversation context. If no sub-agents are available, generate three review prompt files under `{project-root}/.nexus/specs/{active_issue}/`, one per reviewer role below, and HALT. Ask the human to run each in a separate session (ideally a different LLM) and paste back the findings.
