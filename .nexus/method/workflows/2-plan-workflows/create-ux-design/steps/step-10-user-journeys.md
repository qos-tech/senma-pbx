...
### 1b. Draw the Journey as Screens, in Order

**This is the step where the design becomes judgeable.** A journey written as
prose reads as reasonable no matter how bad it is; the same journey drawn as
the screens a person walks through shows a step too many, a dead end or a
missing empty state at a glance.

For the critical journey, draw EVERY screen of it on ONE page, in order, each
under a heading naming the screen and what leads to it. Follow:
...
Use the tokens recorded at step 8, never invented colour, and include the empty
and error states somewhere in the sequence: they are where journeys actually
break and they are the ones prose always omits.

There are no links between the screens, and none is needed. The person scrolls
the flow and sees the whole thing at once, which is what makes a sequence
possible to judge.
...
- Read fully and follow: {project-root}/.nexus/method/core/workflows/advanced-elicitation/workflow.md with the current user journey content
...
- Read fully and follow: {project-root}/.nexus/method/core/workflows/party-mode/workflow.md with the current user journeys
...
- Load `{project-root}/.nexus/method/workflows/2-plan-workflows/create-ux-design/steps/step-11-component-strategy.md`
...
Write content to file immediately after generation. Load `{project-root}/.nexus/method/workflows/2-plan-workflows/create-ux-design/steps/step-11-component-strategy.md` to define component library strategy.
