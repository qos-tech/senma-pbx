...
# upload-security surface it sits on (CSRF, zip-slip, symlink entries,
...
# PermissionPlugin::$alwaysAllow (IndexController / RegisterController).
...
# TASK-0035E9: secure initial admin credential bootstrap lifecycle
# (fresh/idempotent/existing/restore/permissions/retrieval/clear/no-default).
admin-bootstrap-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/admin-bootstrap-smoke-test.sh

# TASK-0035E9: print username + bootstrap plaintext if the operator-local
# secret still exists. Never regenerates. Never touches the database.
bootstrap-admin-credentials:
	@bash scripts/bootstrap-admin-credentials.sh

# TASK-0035E9: remove only the bootstrap plaintext secret file. Does not
# change the database password. Idempotent.
bootstrap-admin-credentials-clear:
	@bash scripts/bootstrap-admin-credentials-clear.sh
...
# TASK-0035E6: operator-facing status detail UX (presenter + badge contract).
status-detail-ux-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/status-detail-ux-smoke-test.sh
...
# TASK-0034I: Inspector (/index.php/inspector) runtime-resource panels.
system-status-runtime-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/system-status-runtime-smoke-test.sh

# TASK-0034I-R1: System Status dashboard (SystemstatusController) — no
# hard-coded :80 self-call, HostResources /proc path, disclosure guards,
# active-call UNKNOWN + restart safety copy, pilot :8080 contract.
systemstatus-dashboard-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/systemstatus-dashboard-smoke-test.sh

# TASK-0034I-R3: PHP 8 runtime warnings from rc.13 pilot navigation
# (apt widget / CallsReport period / Zend HeadLink \$extras).
php8-runtime-warning-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/php8-runtime-warning-smoke-test.sh

# TASK-0034I-R4: app_queue queues.conf seed + AMI get_queues / ip_status
# failure hardening (false vs empty distinction).
queue-runtime-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/queue-runtime-smoke-test.sh

# TASK-0034I-R5: QueuesController ADD/EDIT PHP 8 hardening (default model,
# optional POST radios, getName false-vs-array duplicate check).
queue-management-php8-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/queue-management-php8-smoke-test.sh

# TASK-0034I-R6: Calls Report recording URL (/arquivos not /index.php/arquivos)
# + timezone/date directory resolution (userfield YYYYMMDD / UTC CDR).
recording-report-url-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/recording-report-url-smoke-test.sh

# TASK-0034I-R7: canonical URL contract + route create/duplicate hardening.
global-url-basepath-smoke: ensure-dev-stack
	@set -a; . ./.env; set +a; bash scripts/global-url-basepath-smoke-test.sh
...
# TASK-0035E8: shared call-recording storage (host snep/arquivos) for app
