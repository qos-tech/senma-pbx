...
- The agent roster is every `{project-root}/.nexus/method/agents/*.agent.yaml`
...
Read every agent file matching `{project-root}/.nexus/method/agents/*.agent.yaml`

### 2. Extract Agent Data

Take each agent's `metadata` and `persona` to extract:
