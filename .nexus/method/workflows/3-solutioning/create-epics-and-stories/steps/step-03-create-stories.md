...
# Path Definitions
workflow_path: '{project-root}/.nexus/method/workflows/3-solutioning/create-epics-and-stories'

# File References
thisStepFile: './step-03-create-stories.md'
nextStepFile: './step-04-final-validation.md'
workflowFile: '{workflow_path}/workflow.md'
outputFile: '{docs_dir}/epics.md'

# Task References
advancedElicitationTask: '{project-root}/.nexus/method/core/workflows/advanced-elicitation/workflow.md'
partyModeWorkflow: '{project-root}/.nexus/method/core/workflows/party-mode/workflow.md'
...
- 🔗 **CRITICAL: A story that needs an EARLIER story MUST say so in `dependsOn`**
...
**🔗 WRITE THE ORDER DOWN, in `dependsOn`:**

The rule above says a story may build on EARLIER stories. Say which ones. The
app turns each story into a board issue and reads `dependsOn` to make the new
issue wait on the issues its earlier stories became, so a story that quietly
assumes an earlier one is a card someone starts too soon.

- Each entry names a STORY, never an issue key. The key does not exist yet.
- **QUOTE every id**, exactly as you quote the story's own `id`. Written bare,
  `1.10` reaches the reader as `1.1`, a different story, so the file is refused
  rather than guessed at. `'1.10'` is read as the story you meant.
- Write the story id (`'1.1'`) or its qualified form (`'EPIC-1/1.1'`). Both
  resolve. Use the qualified form when two epics reuse the same story id: a
  bare id that names two stories resolves to neither.
- Only name stories that come BEFORE this one. Forward is still forbidden.
- A story that needs nothing omits the field. An empty list is not a statement.
- Never name a story that is not in this document: it becomes nothing, and the
  app reports it back to the person as declared-but-unresolved.

```yaml
- id: '2.1'
  title: Checkout with a saved card
  statement: 'As a returning shopper, I want my card preselected, so that I skip re-entry.'
  criteria:
    - 'Given a saved card, when I checkout, then it is preselected'
  dependsOn:
    - '1.3'
```

**🚨 THE FRONTMATTER IS WHAT THE APP READS.**

`{outputFile}` has two halves and they must agree. The frontmatter is the typed
source: the app parses `epics`, `stories`, `criteria` and `dependsOn` from
there and from nowhere else. The markdown body below it is the mirror a person
reads. A story written only in the body is invisible to the board, and a
frontmatter the reader refuses makes the WHOLE document invisible, epics and
all, with nothing on screen to say why. Write both halves, and keep the
frontmatter shape exactly as the template shows it.
...
**Depends on:** {earlier story ids, or omit the line}
```
