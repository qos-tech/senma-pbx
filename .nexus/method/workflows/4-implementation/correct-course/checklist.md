...
<critical>This checklist is executed as part of: {project-root}/.nexus/method/workflows/4-implementation/correct-course/workflow.md</critical>
...
<check-item id="6.4">
<prompt>Update the issue files and their specs to reflect approved epic changes; the board follows in the app</prompt>
<action>If epics were added: Author new issue files under `.nexus/issues/` (the app adds their board cards in reaction). An issue file's frontmatter REQUIRES `key`, `title`, `column` and `origin`, and takes `foundIn` when another issue's work exposed this one; the full shape is step 4 of `.nexus/method/workflows/4-implementation/sprint-planning/workflow.md`. An issue missing any required field is refused whole and never becomes a card.</action>
<action>If epics were removed: Note the issues to retire so the app can drop their cards; do not edit the board here</action>
<action>If epics were renumbered: Update issue keys and their specs' `issue` references</action>
<action>If stories were added/removed: Author or retire spec files under `.nexus/specs/<ISSUE-KEY>/` and update each issue's spec list</action>
<action>Never write `.nexus/board.yaml`: the app is the board's only writer and moves cards in reaction to the issue and spec files</action>
<status>[ ] Done / [ ] N/A / [ ] Action-needed</status>
</check-item>
