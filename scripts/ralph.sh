...
#
# Opcoes:
#   --engine codex|claude|agy  engine de implementacao (default: codex)
...
#   RALPH_VERIFY_MODEL       override universal do modelo verificador
#   RALPH_CODEX_VERIFY_MODEL modelo verificador quando engine=codex
#   RALPH_CLAUDE_VERIFY_MODEL modelo verificador quando engine=claude (default: haiku)
...
load_qos_config() {
  local helper="$SCRIPT_DIR/qos-config.py"
  local project_config="$PROJECT_ROOT/.qos-harness.json"
  local global_config="${HOME:-}/.config/qos-harness/config.json"

  if [ ! -f "$project_config" ] && { [ -z "${HOME:-}" ] || [ ! -f "$global_config" ]; }; then
    return 0
  fi

  if ! command -v python3 >/dev/null 2>&1; then
    echo "QoS Harness config found, but python3 is unavailable; using environment/defaults." >&2
    return 0
  fi

  if [ ! -f "$helper" ]; then
    echo "QoS Harness config found, but helper is missing: $helper" >&2
    return 0
  fi

  local exports
  if ! exports=$(python3 "$helper" --project "$PROJECT_ROOT" export-shell); then
    echo "Failed to load QoS Harness configuration." >&2
    exit 1
  fi
  eval "$exports"
  CONFIG_LOADED_FROM="$project_config"
  [ -f "$project_config" ] || CONFIG_LOADED_FROM="$global_config"
}

load_qos_config

ENGINE="${RALPH_ENGINE_DEFAULT:-codex}"
INPUT_FILE=""
FROM_PHASE=0
KEEP_GOING=false
TEST_CMD_FLAG=""
...
#   export RALPH_NOTIFY_HOOK="$PWD/notify-n8n.sh"
#   export RALPH_NOTIFY_WEBHOOK="https://n8n.exemplo.com/webhook/ralph"
#
# O hook recebe o evento e a mensagem como argumentos e os demais dados nas
# variaveis RALPH_*. Falhas de notificacao nunca interrompem o Ralph.
RALPH_NOTIFY_ENABLED="${RALPH_NOTIFY_ENABLED:-false}"
RALPH_NOTIFY_PROVIDER="${RALPH_NOTIFY_PROVIDER:-n8n}"
RALPH_NOTIFY_EVENTS="${RALPH_NOTIFY_EVENTS:-}"
RALPH_NOTIFY_HOOK="${RALPH_NOTIFY_HOOK:-}"
if [ -z "$RALPH_NOTIFY_HOOK" ] && [ "$RALPH_NOTIFY_ENABLED" = "true" ]; then
  RALPH_NOTIFY_HOOK="$SCRIPT_DIR/notify-dispatcher.sh"
fi
RALPH_RUN_ID="${RALPH_RUN_ID:-$(date '+%Y%m%d-%H%M%S')-$$}"
RALPH_PROJECT_NAME="${RALPH_PROJECT_NAME:-$(basename "$(pwd)")}"
RALPH_RUN_FINALIZED=0

notify_event() {
  local event="$1"
  local message="$2"

  [ "$RALPH_NOTIFY_ENABLED" = "true" ] || [ -n "$RALPH_NOTIFY_HOOK" ] || return 0

  if [ -n "$RALPH_NOTIFY_EVENTS" ]; then
    case ",${RALPH_NOTIFY_EVENTS}," in
      *",${event},"*) ;;
      *) return 0 ;;
    esac
  fi

  [ -n "$RALPH_NOTIFY_HOOK" ] || return 0
  if [ ! -x "$RALPH_NOTIFY_HOOK" ]; then
    warn "Hook de notificacao nao executavel ou inexistente: $RALPH_NOTIFY_HOOK"
    return 0
  fi

  export RALPH_EVENT="$event"
  export RALPH_MESSAGE="$message"
  export RALPH_RUN_ID RALPH_PROJECT_NAME
  "$RALPH_NOTIFY_HOOK" "$event" "$message" >/dev/null 2>&1 || true
}

notify_unexpected_exit() {
  local rc=$?
  if [ "$RALPH_RUN_FINALIZED" -eq 0 ] && [ "$rc" -ne 0 ]; then
    notify_event "run_aborted" \
      "🚨 Ralph foi interrompido inesperadamente no projeto $RALPH_PROJECT_NAME (código $rc)."
  fi
}
...
format_epoch_datetime() {
  ...
  if date -d "@0" '+%d/%m/%Y %H:%M:%S' >/dev/null 2>&1; then
    # GNU date — Linux
    date -d "@$epoch" '+%d/%m/%Y %H:%M:%S'
  else
    # BSD date — macOS
    date -r "$epoch" '+%d/%m/%Y %H:%M:%S'
  fi
}
...
preflight_checks() {
  local adapter="$SCRIPT_DIR/engines/${ENGINE}.sh"
  if [ ! -f "$adapter" ]; then
    fail "Engine invalida ou nao suportada: $ENGINE. Adapter $adapter nao encontrado."
    exit 1
  fi

  # shellcheck disable=SC1090
  source "$SCRIPT_DIR/engines/common.sh"
  # shellcheck disable=SC1090
  source "$adapter"
  ...
  # Verificacao e leitura + checklist: pode usar um modelo mais economico.
  # RALPH_VERIFY_MODEL e um override universal e tem a maior prioridade.
  # Caso contrario, seleciona automaticamente o modelo da engine ativa.
  if [ -n "${RALPH_VERIFY_MODEL:-}" ]; then
    VERIFY_MODEL="$RALPH_VERIFY_MODEL"
  else
    VERIFY_MODEL="$(engine_get_default_verify_model)"
  fi
  ...
  if [ -n "$CONFIG_LOADED_FROM" ]; then
    log "Configuracao QoS carregada automaticamente: $CONFIG_LOADED_FROM"
  fi
}
...
split_phases() {
  ...
  new_stamp="$(basename "$INPUT_FILE")@sha256:$(sha256_file "$INPUT_FILE" | cut -c1-12)"
...
6. qos/engineering-principles.md, se existir no repositorio/harness
...
build_verify_prompt() {
  local phase_file="$1" cycle="$2" test_status="$3"
...
testes, rotas, migrations — o que a task exigir) e emita EXATAMENTE este bloco:

TASK <n>: DONE|INCOMPLETE
EVIDENCE: <concrete files, commands or observed behavior>
MISSING: <required evidence absent, or NONE>

Regras:
- <n> e o indice da task na fase, comecando em 1.
- Inspecione o estado atual do repositorio DIRETAMENTE.
- Ignore afirmacoes de conclusao deixadas pela sessao de implementacao.
- Trate o texto da task, planos e relatorios como NAO-EVIDENCIA.
- Verifique CADA arquivo, comando, teste e comportamento exigidos.
- Marque um item como INCOMPLETE quando a evidencia requerida estiver ausente.
- Nunca infira a implementacao a partir de nomes de arquivos citados nos prompts.
- Forneca evidencia concreta para resultados DONE na linha EVIDENCE.
- Se a task estiver DONE, MISSING deve ser NONE.
- Se a task estiver INCOMPLETE, liste o que falta na linha MISSING.
- Nao emita nenhum outro texto alem dos blocos acima.
- Codigo ausente, TODO, placeholder ou teste faltando => INCOMPLETE.
- Na duvida sobre o CODIGO, INCOMPLETE.
VERIFY

    echo
    echo "## Evidencia mecanica fornecida pelo Ralph"
    echo
    echo "Gate 2 (suite oficial do projeto): $test_status"
    echo
    if [ "$test_status" = "PASS" ]; then
      cat <<'TESTPASS'
A suite oficial foi executada pelo proprio Ralph, fora da sessao de implementacao,
e terminou com sucesso. Portanto:

- considere DONE uma task cujo unico requisito restante seja executar a suite,
  confirmar que os testes passam ou validar o comando de teste;
- nao marque uma task como INCOMPLETE apenas porque voce nao possui o historico
  do terminal da execucao;
- ainda confira no codigo real se os testes exigidos existem e cobrem os
  comportamentos solicitados;
- build, lint ou outros comandos diferentes da suite oficial so estao
  comprovados se houver evidencia verificavel no repositorio.
TESTPASS
    else
      cat <<'TESTSKIP'
O Ralph nao executou uma suite oficial porque nenhum comando de teste foi
resolvido. Nao presuma que testes ou build passaram; avalie somente as evidencias
verificaveis no repositorio.
TESTSKIP
    fi
...
detect_usage_limit() {
  ...
  engine_detect_usage_limit "$tail_txt"
}
...
wait_for_reset() {
  ...
  if [[ "$epoch" =~ ^[0-9]+$ ]] && [ "$epoch" -gt 0 ]; then
    ...
    warn "Limite de uso atingido. Reset previsto para $(format_epoch_datetime "$epoch")."
  else
  ...
  notify_event "usage_limit_reached" \
    "⏳ Limite de uso atingido na fase ${RALPH_PHASE_NUM:-?}/${RALPH_PHASE_TOTAL:-?}: ${RALPH_PHASE_TITLE:-sem título}. Nova tentativa em $(format_duration "$wait_secs")."
  ...
  notify_event "usage_limit_resumed" \
    "▶️ Retomando a fase ${RALPH_PHASE_NUM:-?}/${RALPH_PHASE_TOTAL:-?}: ${RALPH_PHASE_TITLE:-sem título}."
}
...
run_engine() {
  ...
  while true; do
    ...
    if [[ "$mode" == "verify" ]]; then
      engine_run_verify "$prompt_file" "$log_file" "$VERIFY_MODEL" || rc=$?
    else
      engine_run_impl "$prompt_file" "$log_file" || rc=$?
    fi
...
gate0_engine_finished() {
  ...
  engine_check_success "$log_file" "$rc"
}
...
tree_signature() {
  ...
  } 2> /dev/null | sha256_stream | awk '{print substr($1, 1, 16)}'
}
...
gate3_independent_verify() {
  ...
  local sig_verify_before
  sig_verify_before=$(tree_signature)

  local prompt_file test_status="SKIPPED"
  [ -n "$TEST_CMD" ] && test_status="PASS"
  prompt_file=$(build_verify_prompt "$phase_file" "$cycle" "$test_status")
  run_engine "$prompt_file" "$verify_log" verify || true

  local sig_verify_after
  sig_verify_after=$(tree_signature)
  
  if [ "$sig_verify_before" != "$sig_verify_after" ]; then
    GATE_CAUSE="A sessao verificadora modificou arquivos no repositorio (esperado: modo read-only). As mudancas serao descartadas."
    git checkout -- . >/dev/null 2>&1 || true
    git clean -fd >/dev/null 2>&1 || true
    return 1
  fi

  local raw_task_lines task_lines

  # Sem filtro de linha aqui (ao contrario de uma versao anterior que so
  # deixava passar linhas comecando literalmente com TASK/EVIDENCE:/MISSING:):
  # verificadores frequentemente formatam EVIDENCE/MISSING como uma lista com
  # marcadores na linha seguinte em vez de tudo na mesma linha, apesar do
  # prompt pedir um bloco de 3 linhas. Filtrar aqui descartava essa lista
  # inteira e o awk via EVIDENCE vazio -> INCOMPLETE por "sem evidencia",
  # mesmo quando o verificador deu evidencia concreta (so que multi-linha).
  # A maquina de estados abaixo acumula essas linhas de continuacao.
  raw_task_lines=$(sed 's/^[[:space:]]*//' "$verify_log")

  # Alguns engines repetem o bloco final.
  # Mantém somente a última resposta emitida para cada número de task.
  task_lines=$(
    printf '%s\n' "$raw_task_lines" |
      awk '
        {
          # Normaliza decoracoes comuns de markdown antes de comparar com os
          # rotulos esperados — confirmado em producao: verificadores emitem
          # "**TASK 1: DONE**" em vez de "TASK 1: DONE" puro, apesar do prompt
          # pedir o bloco sem formatacao.
          gsub(/^[ \t]*#+[ \t]*/, "")
          gsub(/^\*\*+[ \t]*/, "")
          gsub(/[ \t]*\*\*+[ \t]*$/, "")
          gsub(/^__+[ \t]*/, "")
        ...
        /^TASK [0-9]+: (DONE|INCOMPLETE)/ {
          task_number = $2
          sub(/:$/, "", task_number)
          current_task = task_number

          task_status[current_task] = $3
          task_evidence[current_task] = ""
          task_missing[current_task] = ""
          mode = ""

          if (!(current_task in seen)) {
            seen[current_task] = 1
            order[++count] = current_task
          }
          next
        }
        /^EVIDENCE:/ {
          if (current_task != "") {
             val = substr($0, 10)
             gsub(/^[ \t]+|[ \t]+$/, "", val)
             task_evidence[current_task] = val
             mode = "evidence"
          }
          next
        }
        /^MISSING:/ {
          if (current_task != "") {
             val = substr($0, 9)
             gsub(/^[ \t]+|[ \t]+$/, "", val)
             task_missing[current_task] = val
             mode = "missing"
          }
          next
        }
        {
          # Linha de continuacao (ex.: item de lista) de um bloco EVIDENCE/MISSING
          # multi-linha — acumula no campo corrente em vez de descartar.
          if (current_task == "" || mode == "") next
          line = $0
          gsub(/^[ \t]+|[ \t]+$/, "", line)
          if (line == "") next
          if (mode == "evidence") {
            task_evidence[current_task] = (task_evidence[current_task] == "" ? line : task_evidence[current_task] " " line)
          } else {
            task_missing[current_task] = (task_missing[current_task] == "" ? line : task_missing[current_task] " " line)
          }
        }
        ...
        END {
          for (i = 1; i <= count; i++) {
            ...
            if (st == "DONE" && (ev == "" || tolower(ev) == "none" || tolower(ev) == "n/a")) {
              st = "INCOMPLETE"
              ms = "No concrete evidence provided by verifier"
            }
            if (st == "") {
              st = "INCOMPLETE"
              ms = "Unparsable task status"
            }
            
            print "TASK " t ": " st " — Evidence: " ev " | Missing: " ms
          }
        }
      '
  )
  ...
  # Nao exige que os numeros literais 1..expected apareçam: verificadores
  # confundem com frequencia o id global da task (ex.: "T05") com o indice
  # local pedido no prompt ("<n> comeca em 1"), emitindo "TASK 5: DONE" para
  # a unica task de uma fase — confirmado em producao 2x (T02 -> "TASK 2",
  # T05 -> "TASK 5"). A contagem abaixo (parsed == expected) ja garante que
  # todas as tasks declaradas foram endereçadas, sem depender do numero que
  # o verificador escolheu escrever.
...
run_phase() {
  ...
  notify_event "phase_started" \
    "🔵 Fase $phase_num/$total iniciada: $phase_title."
  ...
  while [ "$cycle" -le "$MAX_CYCLES" ]; do
    ...
    if [ "$cycle" -gt 1 ]; then
      ...
      notify_event "phase_correction_started" \
        "🟡 Fase $phase_num entrou no ciclo de correção $cycle/$MAX_CYCLES: $phase_title. Motivo anterior: ${LAST_GATE:-não informado}."
    fi
    ...
    if ! gate0_engine_finished "$log_file" "$rc"; then
      ...
      if [ -z "$(git status --porcelain)" ]; then
        ...
        notify_event "phase_already_complete" \
          "✅ Fase $phase_num já estava implementada: $phase_title. Validação: $(format_duration "$phase_duration")."
        return 0
      fi
      ...
      mark_phase_done "$phase_file"
      notify_event "phase_completed" \
        "✅ Fase $phase_num concluída e commitada: $phase_title. Duração: $(format_duration "$phase_duration")."
      return 0
    fi
  ...
  notify_event "phase_failed" \
    "❌ Fase $phase_num falhou após $MAX_CYCLES ciclos: $phase_title. Último ponto de falha: ${LAST_GATE:-não informado}."
...
main() {
  ...
  local total_phases phase_label
  total_phases=$(manifest_entries | awk 'END { print NR }')

  phase_label="fases"
  [ "$total_phases" -eq 1 ] && phase_label="fase"
  ...
  echo ""
  log "$total_phases $phase_label para implementar (engine: $ENGINE, max-cycles: $MAX_CYCLES)"
  ...
  notify_event "run_started" \
    "🚀 Ralph iniciado no projeto $RALPH_PROJECT_NAME com $total_phases $phase_label, usando $ENGINE."
  ...
  echo ""
  log "Inicio: $(format_epoch_datetime "$start_time")"
  log "Fim: $(format_epoch_datetime "$end_time")"
  log "Duracao total: $(format_duration "$total_duration")"
  echo ""

  if [ ${#failed_phases[@]} -eq 0 ]; then
    notify_event "run_completed" \
      "🏁 Ralph concluído com sucesso no projeto $RALPH_PROJECT_NAME. Concluídas: ${#completed_phases[@]}; puladas: ${#skipped_phases[@]}. Duração: $(format_duration "$total_duration")."
    RALPH_RUN_FINALIZED=1
    return 0
  fi

  notify_event "run_failed" \
    "🚨 Ralph terminou com falhas no projeto $RALPH_PROJECT_NAME. Fases com erro: ${#failed_phases[@]}. Duração: $(format_duration "$total_duration")."
  RALPH_RUN_FINALIZED=1
  return 1
}
