...
- `installed_path` = `{project-root}/.nexus/method/workflows/4-implementation/correct-course`
- `checklist` = `{installed_path}/checklist.md`
- `default_output_file` = `{docs_dir}/sprint-change-proposal-{date}.md`

### Input Files

| Input | Path | Load Strategy |
|-------|------|---------------|
| PRD | `{docs_dir}/*prd*.md` (whole) or `{docs_dir}/*prd*/*.md` (sharded) | FULL_LOAD |
| Epics | `{docs_dir}/*epic*.md` (whole) or `{docs_dir}/*epic*/*.md` (sharded) | FULL_LOAD |
| Architecture | `{docs_dir}/*architecture*.md` (whole) or `{docs_dir}/*architecture*/*.md` (sharded) | FULL_LOAD |
| UX Design | `{docs_dir}/*ux*.md` (whole) or `{docs_dir}/*ux*/*.md` (sharded) | FULL_LOAD |
| Tech Spec | `{docs_dir}/*tech-spec*.md` (whole) | FULL_LOAD |
...
**Fuzzy matching**: Be flexible with document names, since users may use variations like `prd.md`, `bmm-prd.md`, `product-requirements.md`, etc.
